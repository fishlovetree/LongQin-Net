...
layui.define(['form', 'jquery', 'layer', 'cascader', 'tags'], function (exports) {
    ...
    Class.prototype.events = function () {
        ...
        $('body').on('click', '.layui-btn-commit', function (e) {
            ...
            var data = JSON.stringify(options.data, null, 4);
            if (!data) {
                layer.msg("表单不能为空");
                return;
            }
            var isValid = form.validate('#formid');
            if (!isValid) {
                layer.msg("请填写正确的表单ID");
                return;
            }
            isValid = form.validate('#formname');
            if (!isValid) {
                layer.msg("请填写正确的表单名称");
                return;
            }
            ...
            $.ajax({
                type: 'post',
                url: commitUrl,
                data: { "Id": $("#id").val(), "TableName": formid, "FormName": formname, "JsonData": data, "IsApproval": isApproval },
                success: function (result) {
                    if (result && result.success) {
                        location.href = '/formdesigner/formdesign/list';
                    }
                    else {
                        layer.msg("表单ID已存在");
                    }
        ...
        // 流程设计页面提交表单
        $('body').on('click', '.layui-btn-subcommit', function (e) {
            ...
            $.ajax({
                ...
                success: function (result) {
                    if (result && result.success) {
                        // 回调流程设计器
                        parent.mflow.resetForm(formname);
                        parent.layer.close($("#layerIndex").val());
                    }
                }
            });
        })
   ...
   * @param {*} field 
   */
    Class.prototype.edit = function (dataJson) {
        var that = this, options = that.config;
        try {
            var data = dataJson;
            options.data = JSON.parse(data);
    ...
    var uploadFile = function(selectName, accept, size) {
        layui.upload.render({
            elem: '#' + selectName + '_btn', // 绑定多个元素
            url: '/Work/WorkFlow/UploadFile', // 此处配置你自己的上传接口即可
            accept: accept,
            size: size,
            done: function (res, index, upload) {
                if (res && res.success) {
                    layer.msg('上传成功');
                    $('input[name=' + selectName + ']').val(res.data);
                }
                else {
                    layer.msg('上传失败');
                }
    ...
    Class.prototype.renderComponent = function (data, elem) {
        ...
        for (let index = 0; index < data.length; index++) {
            ...
            that.renderComponentExtend(element, index);
    ...
    // 获取组件代码-特殊组件渲染
    var renderComponentExtend = function (element, index) {
        ...
        switch (element.tag) {
            ...
            case 'slider':
                ...
                slider.each(function (index, elem) {
                    var that = $(this),
                        ...
                        disabled = that.data('disabled') || false;
            ...
            case 'cascader':
                ...
                if ($(caItem).val() != 'undefined') {
                    $(caItem).parent().find('.el-input__inner').val($(caItem).val());
                }
                else {
                    $(caItem).parent().find('.el-input__inner').val('');
                }
    ...
    // 获取组件代码-特殊组件渲染
    Class.prototype.renderComponentExtend = renderComponentExtend;
    ...
    var renderComponentItem = {
            ...
            render: function (data) {
                ...
                if (data.options.length) {
                    for (let index = 0; index < data.options.length; index++) {
                        ...
                        if (!data.default && index === 0) {
                            html += 'checked=""';
                        }
                        if (elem.value == data.default) {
                            html += 'checked=""';
                        }
                        if (data.disabled) {
                            html += 'disabled=""';
            ...
            render: function (data) {
                ...
                if (data.options.length) {
                    for (let index = 0; index < data.options.length; index++) {
                        ...
                        if (data.default && data.default.indexOf(elem.value) >= 0) {
                            html += 'checked=""';
                        }
                        if (data.disabled) {
                            html += 'disabled=""';
        ...
        colorpicker: {
            render: function (data) {
                var html = getBeforeItem(data);
                html += getBeforeLabel(data.labelhide, data);
                html += '<input class="layui-input layui-hide ' + data.name + '" name="' + data.name + '" value="' + data.default + '"/>';
                html += '<div lay-colorpicker="' + data.name + '" data-value="' + data.default + '"';
                if (data.readonly) {
                    html += 'disabled=""';
                }
        ...
        slider: {
            render: function (data) {
                ...
                html += getBeforeLabel(data.labelhide, data);
                html += '<input class="layui-input layui-hide ' + data.name + '" name="' + data.name + '" value="' + data.default + '"/>';
                html += '<div class="lay-slider" lay-slider="' + data.name + '" data-default="' + data.default + '" data-theme="' + data.data_theme + '"';
        ...
        rate: {
            render: function (data) {
                ...
                html += getBeforeLabel(data.labelhide, data);
                html += '<input class="layui-input layui-hide ' + data.name + '" name="' + data.name + '" value="' + data.default + '"/>';
                html += '<div lay-rate="' + data.name + '" data-default="' + data.default + '" data-class="' + data.name + '" data-theme="' + data.data_theme + '"';
        ...
        switch: {
            render: function (data) {
                ...
                html += getBeforeLabel(data.labelhide, data);
                if (data.default == 1) {
                    html += '<input type="checkbox" name="' + data.name + '" lay-skin="switch" checked ';
                }
                else {
                    html += '<input type="checkbox" name="' + data.name + '" lay-skin="switch" ';
                }
                if (data.readonly) {
                    html += 'disabled=""';
                }
        ...
        cascader: {
            render: function (data) {
                var html = getBeforeItem(data);
                html += getBeforeLabel(data.labelhide, data);
                html += '<input id="' + data.name + '" name="' + data.name + '" class="layui-input" data-parents="' + data.data_parents + '" value="' + data.default + '" data-value="' + data.default + '">';
                html += '</div></div>';
                return html;
            }
        },
        ...
        upload: {
            render: function (data) {
                ...
                html += '>';
                if (!data.readonly) {
                    html += '<button type="button" class="layui-btn layui-upload-btn" id="' + data.name + '_btn" lay-choose="' + data.name + '" data-type="normal" >';
                    html += '<i class="layui-icon layui-icon-windows"></i>选择</button>';
                }

                if (data.default) {
                    html += '<a class="layui-btn" href="' + data.default + '" >下载</a>';
                }
    ...
    Class.prototype.renderComponentItem = renderComponentItem;
    ...
    //渲染普通组件
    MODULE_FORMDESIGN_NAME.renderComponentItem = renderComponentItem;

    //渲染特殊组件
    MODULE_FORMDESIGN_NAME.renderComponentExtend = renderComponentExtend;
